...
import { getStemCalculator, type StemCalculatorConfig } from '../../utils/stemCalculators';
import type { CalculatorResult } from '../../types';

function getInitialValues(config: StemCalculatorConfig, useExample = false) {
...
export function StemCalculatorPage({ calculatorId }: Props) {
  const { slug } = useParams();
  const config = getStemCalculator(calculatorId || slug);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="card max-w-md text-center">
          ...
          <Link to="/calculators" className="btn-primary inline-flex">
            Browse calculators
          </Link>
        </div>
      </div>
    ...
    <CalculatorLayout calculator={calculator} result={result}>
      <div className="space-y-5">
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
          <p className="text-sm text-blue-900 leading-relaxed">{config.explanation}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <button type="button" onClick={handleCalculate} className="btn-primary flex-1 sm:flex-none">
            Calculate
          </button>
          <button type="button" onClick={handleExample} className="btn-secondary">
            Load Example
          </button>
