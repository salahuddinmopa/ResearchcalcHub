...
const calc = getCalculatorById('variance')!;
...
export function VariancePage() {
  ...
  const handleCalculate = () => {
    ...
    setResult({
      summary: [
        { label: `${mode === 'sample' ? 'Sample' : 'Population'} Variance`, value: r.variance.toFixed(6), highlight: true },
        { label: 'Mean', value: r.mean.toFixed(6) },
        { label: 'Sum of Squared Differences', value: r.sumSquaredDifferences.toFixed(6) },
        { label: 'Standard Deviation', value: r.standardDeviation.toFixed(6) },
        { label: 'Count', value: r.count.toString() },
        { label: 'Divisor', value: r.divisor.toString() },
      ],
      interpretation: `The ${mode} variance is ${r.variance.toFixed(4)}. Larger variance indicates greater spread around the mean (${r.mean.toFixed(4)}).`,
        ...
        <div className="flex flex-wrap gap-4">
          {(['sample', 'population'] as const).map(option => (
            <label key={option} className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm font-medium text-slate-700 capitalize">{option} variance</span>
